...
Plan → Execute → Evaluate → Refine · Human-supervised RSI

[四层架构](#系统架构) · [RSI 闭环](#rsi-跨轮闭环) · [快速开始](#快速开始) · [执行流程](docs/EXECUTION_FLOW.md) · [实验结果](#已记录的实验结果) · [文档导航](docs/README.md)
...
| 层次 | 负责什么 | 跨层产物 | 当前实现 |
| --- | --- | --- | --- |
| **L1 · RSI 跨轮改进** | 证据汇总、诊断、修订审核、后续验证 | 带适用条件的经验与修订记录 | 人工监督；提供复盘模板与案例 |
...
RSI 的输入是本轮证据，输出是**经过审核、带适用条件的下一轮修订**。经验由规划者显式选用；每一轮都重新读取场景与状态、生成计划并验证，不在执行过程中自动修改目标流。
...
[RSI 机制与实现状态](docs/RSI.md) · [真实修订案例](experiments/2026-09-17/rsi_review.md) · [复盘模板](docs/templates/RSI_REVIEW.md) · [可编辑闭环图](docs/figures/rsi_cycle.drawio)
