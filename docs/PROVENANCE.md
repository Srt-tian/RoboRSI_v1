...
Version 0.3 adds original typed-judgment interfaces inspired by publicly documented System One patterns, an optional TypeSafe HTTP adapter, phase expectations, abstention, semantic-context checks and episode-disjoint evaluation. No external model implementation or weights are copied. The HTTP adapter was contract-tested with mocked responses, not verified against a paid live endpoint. Calibration fixtures and demo replies are explicitly synthetic.

The 2026-09-22 simulation archive contains 576 kinematic proxy episodes with analytic proposals and injected delays. It is neither a Piper digital twin nor evidence for a trained Jev/GPT/RSI policy. Videos are rendered from those saved simulated states. All 269,616 simulated samples, event logs and archive hashes are available locally; historical hardware records remain separate. See [simulation assumptions](SIMULATION.md), [Jev survey](JEV_RESEARCH_2026_09.md) and [research proposal](RESEARCH_PROPOSAL.md).
