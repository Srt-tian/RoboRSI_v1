...
- 已反证：90 父状态的队列机制先导未通过训练门槛；事后差分控制暴露固定偏置特例。新增 1,080 条分支、79,443 个控制步、16 段归档状态视频；没有新增模型。
...
参考文献已核查一手页面；新增 RTC、2026 年 8 月 ARLI、信息价值动作块与乱序测量估计，明确队列条件化已有先例。使用 AI 辅助了研究设计、实现、分析和起草；正式稿应依会议政策由人类作者核对、负责并如实披露。
